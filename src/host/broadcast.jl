...
@inline function _copyto!(dest::AbstractArray, bc::Broadcasted)
    ...
    if ndims(dest) == 1 || (isa(IndexStyle(dest), IndexLinear) &&
        ...
        broadcast_kernel_linear(get_backend(dest))(dest, bc, ndrange = length(size(dest)) > 0 ? length(dest) : 1)
    else
        broadcast_kernel_cartesian(get_backend(dest))(dest, bc, ndrange = sz = length(size(dest)) > 0 ? size(dest) : (1,))
    end
